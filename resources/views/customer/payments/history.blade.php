                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                <div>{{ ($payment->paid_at ?? $payment->created_at)->format('d M, Y') }}</div>
                                <div class="text-xs text-gray-500">{{ ($payment->paid_at ?? $payment->created_at)->format('h:i A') }}</div>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-900">
                                <div class="font-medium">{{ $payment->payment_type }}</div>
                                <div class="text-xs text-gray-500">
                                    {{ implode(' • ', array_map(fn ($i) => $i['description'], $payment->lineItems())) }}
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-sm">
                                <a href="{{ route('customer.payments.receipt', $payment) }}" target="_blank" class="text-primary-600 hover:underline">View</a>
                            </td>
                        </tr>
                        @empty
                        <tr>
