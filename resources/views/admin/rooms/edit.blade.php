            ...
            <div class="mb-4 bg-amber-50 border-2 border-amber-200 rounded-lg p-4">
                <div class="flex items-start">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-xs text-amber-700 mt-1">This room has {{ $room->capacity }} capacity and already has
                            {{ $room->beds->count() }} beds. Cannot add more beds.</p>
                    </div>
                </div>
            </div>
        @endif
                    ...
                    @forelse($room->beds as $bed)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                ₹{{ number_format($bed->monthly_rent) }}</td>
                            <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm">
                                <form action="{{ route('admin.beds.update-status', $bed) }}" method="POST">
                                    ...
                                    <select name="status" onchange="this.form.submit()"
                                        ...
                                        <option value="reserved" {{ $bed->status == 'reserved' ? 'selected' : '' }}>Reserved
                                        </option>
