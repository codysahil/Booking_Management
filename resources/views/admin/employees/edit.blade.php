    ...
    <div class="max-w-4xl mx-auto">
        <form action="{{ route('admin.employees.update', $employee) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="address" rows="3" required
                            class="w-full rounded-lg border-gray-300 focus:border-primary-500 focus:ring-primary-500">{{ $employee->address }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-6">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Employee Photo (Leave blank to keep
                            current)</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">ID Proof (Leave blank to keep
                            current)</label>
                        @if($employee->proof_path)
                            <div class="mb-2">
                                <a href="{{ Storage::url($employee->proof_path) }}" target="_blank"
                                    class="text-sm text-primary-600 hover:underline">View Current Proof</a>
                            </div>
                        @endif
